...
def solve(frame):
    ...
    cnt, cnt_position = get_contour(img)

    if (cnt_position is not None):
        cnt_img = img.copy()
        ...
        if (check_sudoku(warp_img)):
            ...
            if (len(flat_solved_board_nums) != 0):
                text_img = display_numbers(warp_img, flat_solved_board_nums)
                ...
                temp = img.copy()
                combined = cv2.addWeighted(temp, 0.7, inv, 1, 0)
